import React from 'react'

const SEOServicesYouCanExpectFromBreezendTechs = () => {
  return (
    <>
      <section className="service-section">
        <div className="pd_top_70" />
        <div className="container">
          <div className="row">
            <div className="col-xl-6 col-lg-6 col-md-12 col-sm-12 col-xs-12">
              <div className="title_all_box style_three  dark_color">
                <div className="title_sections three left">
                  <h3>SEO Services You Can Expect From BreezendTechs</h3>
                  <p>
                    It is a long established fact that a reader will be distracted
                    by the readable content of a page when looking at its layout.
                    The point of using Lorem Ipsum is that it has a more-or-less
                    normal distribution of letters, as opposed to using 'Content
                    here, content here', making it look like readable English.
                  </p>
                  <p>
                    It is a long established fact that a reader will be distracted:
                  </p>
                  <div className="list_item_box style_two style_list">
                    <ul className="list-inline">
                      <li className="list_items pd_bottom_10">
                        <small className="d-flex align-items-center">
                          <span className="icon_bx">
                            <i className=" icon-checked" />
                          </span>
                          <a
                            className="nav_link"
                            href="#"
                            target='"_blank"'
                            rel='"nofollow"'
                          >
                            End to end visibility of time metrics for the hybrid
                            workforce{" "}
                          </a>
                        </small>
                      </li>
                      <li className="list_items pd_bottom_10">
                        <small className="d-flex align-items-center">
                          <span className="icon_bx">
                            <i className=" icon-checked" />
                          </span>
                          <a
                            className="nav_link"
                            href="#"
                            target='"_blank"'
                            rel='"nofollow"'
                          >
                            Workload Balancing between individuals and teams{" "}
                          </a>
                        </small>
                      </li>
                      <li className="list_items pd_bottom_10">
                        <small className="d-flex align-items-center">
                          <span className="icon_bx">
                            <i className=" icon-checked" />
                          </span>
                          <a
                            className="nav_link"
                            href="#"
                            target='"_blank"'
                            rel='"nofollow"'
                          >
                            Create Automated timesheets concentrated facilities{" "}
                          </a>
                        </small>
                      </li>
                      <li className="list_items pd_bottom_10">
                        <small className="d-flex align-items-center">
                          <span className="icon_bx">
                            <i className=" icon-checked" />
                          </span>
                          <a
                            className="nav_link"
                            href="#"
                            target='"_blank"'
                            rel='"nofollow"'
                          >
                            Integrate ProHance data within payroll systems{" "}
                          </a>
                        </small>
                      </li>
                      <li className="list_items pd_bottom_10">
                        <small className="d-flex align-items-center">
                          <span className="icon_bx">
                            <i className=" icon-checked" />
                          </span>
                          <a
                            className="nav_link"
                            href="#"
                            target='"_blank"'
                            rel='"nofollow"'
                          >
                            Transform processes based on work patterns{" "}
                          </a>
                        </small>
                      </li>
                      <li className="list_items pd_bottom_10">
                        <small className="d-flex align-items-center">
                          <span className="icon_bx">
                            <i className=" icon-checked" />
                          </span>
                          <a
                            className="nav_link"
                            href="#"
                            target='"_blank"'
                            rel='"nofollow"'
                          >
                            It is a long established fact that a reader will be
                            distracted.{" "}
                          </a>
                        </small>
                      </li>
                      <li className="list_items pd_bottom_10">
                        <small className="d-flex align-items-center">
                          <span className="icon_bx">
                            <i className=" icon-checked" />
                          </span>
                          <a
                            className="nav_link"
                            href="#"
                            target='"_blank"'
                            rel='"nofollow"'
                          >
                            Transform processes based on work patterns{" "}
                          </a>
                        </small>
                      </li>
                    </ul>
                  </div>
                </div>
              </div>
            </div>
            <div className="col-xl-6 col-lg-6 col-md-12 col-sm-12 col-xs-12">
              <div className="service_box style_two dark_color">
                <div className="image_boxes style_four">
                  <div className="one">
                    <img
                      src="/images/service/seo-service-provide.jpg"
                      className="img-fluid"
                      alt="about"
                    />
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="pd_bottom_50" />
      </section>
    </>
  )
}

export default SEOServicesYouCanExpectFromBreezendTechs